...
if(!testUser) return res.status(400).send('The user does not exist')
console.log(bcrypt.compareSync(password, testUser.passwordHash))
if(bcrypt.compareSync(password, testUser.passwordHash)) return res.status(201).send(testUser)
...
export async function  signupUser(req, res){
    ...
    const testUser = await User.findOne({userEmail:userEmail})
    if(testUser) return res.status(409).send('user already exists')
